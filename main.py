...
def download_via_pafy(sender, app_data, user_data):
    url = dpg.get_value(user_data[0])
    path = dpg.get_value(user_data[1])
    try:
        yt = pafy.new(url)
        ...
        return
    print(f"Downloading [{yt.title}] (by {yt.author})")
    yt.getbest().download(filepath=path)
    print('Operation completed!')


dpg.create_context()
with dpg.window(label='Input video link below and click on the button', width=494, height=120, pos=(0, 50),
no_collapse=True, no_close=True, no_move=True, no_resize=True, no_scrollbar=True, no_background=True):
    ...
    dpg.add_button(label='Download video with pafy (slow)', callback=download_via_pafy, user_data=tb_list)
